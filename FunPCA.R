...
plot_pca <- function(dpca,
  ...
  if (!is.null(color_by)) {
    ...
    # pca_df$color_data <- as.factor(df[[color_by]])
  }
  ...
  if (!is.null(color_by)) {
    color_scale <- if (is.numeric(df[[color_by]])) {
      scale_color_viridis_c(name = color_by, option = color_palette, direction = -1)
    } else {
      scale_color_viridis_d(name = color_by, option = color_palette, direction = -1)
    }
    pa <- pa + color_scale
  }
  ...
  pa <- pa + 
    ...
    coord_cartesian(xlim = c(-9, 5), ylim = c(-3, 4)) +
    ggtitle(title) +
...
# Plot PCA Clusters -------------------------------------------------------
plot_clusters <- function(clusters, df, title = "Hierarchical Clustering", legend = FALSE,
  ...
  if (is.numeric(color_groups)) {
    ...
    # color_scale <- colorRampPalette(c("#B23AEE", "#00B2EE", "#EEAD0E"))
  } else {
  ...
  if (is.numeric(ellipse_groups)) {
    ...
    # fill_scale <- colorRampPalette(c("#B23AEE", "#00B2EE", "#EEAD0E"))
  } else {
  ...
  if (legend) {
    pc <- pc +
      guides(
        ...
        fill = guide_legend(title = ellipse_by_label, override.aes = list(alpha = 0.3)),
...
vp_by_var <- function(data, cluster_col, separate = FALSE, center_line = NULL, legend = TRUE) {
  ...
  if(0){df_long$Variable <- factor(df_long$Variable,
                             levels = c("Rinput", 
                                        "Tau" , 
                                        "I thres.", 
                                        "AP HW" , 
  ...
  if (!separate) {
    ...
    plot <- ggplot(df_long, aes(x = .data[[cluster_col]], y = Value, fill = .data[[cluster_col]])) +
      ...
      # stat_summary(fun = center_fun, geom = "crossbar", width = 0.4, color = "black", linewidth = 0.2)+
      geom_boxplot(width = 0.1, outlier.shape = NA, color = "black", alpha = 0.5)+
      geom_jitter(width = 0.15, alpha = 0.4, shape = 21, color = "black") +
      facet_wrap(~Variable, nrow = 6,scales = "free_y",labeller = label_value) +
      custom_colors +
      theme_minimal() +
      theme(
...
kmeans_plotly_clusters <- function(data,
  ...
  if (auto_select) {
    ...
    print(
      ggplot(data.frame(Clusters = 1:max_clusters, WSS = wss), aes(x = Clusters, y = WSS)) +
        ...
        scale_x_continuous(breaks = 1:max_clusters) +
        labs(title = "Elbow Method", x = "Clusters", y = "Within-Cluster Sum of Squares") +
        theme_minimal()
    )
    optimal_clusters <- as.integer(readline(prompt = "Enter the optimal number of clusters: "))
  }
  ...
  if (pca) {
    ...
    pca_data <- as.data.frame(scaled_data)[, 1:3]
    colnames(pca_data) <- c("PC1", "PC2", "PC3")
  }
  
  pca_data$Cluster <- as.factor(kmeans_result$cluster)
  if (!is.null(symbol_by)) pca_data[[symbol_by]] <- data[[symbol_by]]
  if (!is.null(color_by)) pca_data$Color <- as.factor(data[[color_by]])
  else pca_data$Color <- pca_data$Cluster
  
  # Symbols
  default_symbol_map <- if (!is.null(symbol_by)) {
    c("Phasic" = "circle", "Tonic" = "square")
  } else {
    "circle"
  }
  
  # Colors
  unique_colors <- unique(pca_data$Color)
  ...
  generate_ellipsoid <- function(center, cov_matrix, num_points = 40) {
    ...
    sphere <- cbind(c(x), c(y), c(z))
    
    eig <- eigen(cov_matrix)
    trans <- eig$vectors %*% diag(sqrt(abs(eig$values)))
    ellipsoid <- t(apply(sphere, 1, function(p) drop(trans %*% p) + center))
    
    list(
      x = matrix(ellipsoid[, 1], nrow = num_points),
      y = matrix(ellipsoid[, 3], nrow = num_points),
      z = matrix(ellipsoid[, 2], nrow = num_points)
    )
  }
  ...
  for (i in seq_len(nrow(pca_data))) {
    row <- pca_data[i, ]
    symbol <- if (!is.null(symbol_by)) default_symbol_map[[as.character(row[[symbol_by]])]] else default_symbol_map
    ...
    plot <- plot %>%
      add_trace(
        ...
        marker = list(size = 6, color = color, symbol = symbol),
        showlegend = FALSE
      )
  }
  ...
  for (cl in unique(pca_data$Cluster)) {
    cluster_pts <- pca_data[pca_data$Cluster == cl, c("PC1", "PC2", "PC3")]
    if (nrow(cluster_pts) > 2) {
      center <- colMeans(cluster_pts)
      cov_matrix <- cov(cluster_pts)
      ellipsoid <- generate_ellipsoid(center, cov_matrix)
      color <- cluster_color_map[[as.character(cl)]]
      
      plot <- plot %>%
        add_surface(x = ellipsoid$x, y = ellipsoid$y, z = ellipsoid$z,
                    showscale = FALSE, opacity = 0.7,
                    colorscale = list(c(0, 1), c(color, color))) %>%
        add_surface(x = ellipsoid$x, y = ellipsoid$y,
  ...
  plot <- plot %>%
    layout(
      scene = list(
        xaxis = list(title = "PC1", showgrid = grid),
        yaxis = list(title = "PC3", showgrid = grid),
        zaxis = list(title = "PC2", showgrid = grid),
        aspectmode = if (grid) "data" else "cube"
      )
    )
  
  print(plot)
  return(list(kmeans_result = kmeans_result, clustered_data = pca_data))
}
